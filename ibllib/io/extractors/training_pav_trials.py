    ...
    **Optional:** saves _ibl_trials.feedbackType.npy
    ...
    def _extract(self):
        ...
        for i, t in enumerate(self.bpod_trials):
            ...
            state_names = ['reward_delivery', 'reward_delivery_omitted']
            ...
            #if outcome == 'correct':
            if outcome == 'reward_delivery':
                feedbackType[i] = 1
            #elif outcome in ['error', 'no_go']:
            elif outcome == 'reward_delivery_omitted':
                feedbackType[i] = -1
        return feedbackType
    ...
    **Optional:** saves _ibl_trials.rewardVolume.npy
    ...
    def _extract(self):
        trial_volume = [x['reward_amount']
                        if x['trial_type'] in [0, 1] else 0 for x in self.bpod_trials]
...
class FeedbackTimes(BaseBpodTrialsExtractor):
    ...
    **Optional:** saves _ibl_trials.feedback_times.npy
    ...
    def get_feedback_times_lt5(session_path, task_collection='raw_behavior_data', data=False):
        ...
        rw_times = [tr['behavior_data']['States timestamps'].get('reward_delivery', [[np.nan]])[0][0] for tr in data]
        omit_times = [tr['behavior_data']['States timestamps'].get('reward_delivery_omitted', [[np.nan]])[0][0] for tr in data]
        ...
        assert sum(np.isnan(rw_times) & np.isnan(omit_times)) == 0
        merge = np.array([np.array(times)[~np.isnan(times)] for times in
                          zip(rw_times, omit_times)]).squeeze()
    ...
    def get_feedback_times_ge5(session_path, task_collection='raw_behavior_data', data=False):
        ...
        rw_times, stim_times, merge = [np.zeros([len(data), ]) for _ in range(3)]
        ...
        for ind, tr in enumerate(data):
            ...
            rw_times[ind] = tr['behavior_data']['States timestamps']['reward_delivery'][0][0]
            # get the error sound only if the reward is nan
            # stim_times[ind] = st[-1] if st.size >= 2 and np.isnan(rw_times[ind]) else np.nan
            stim_times[ind] = st[0] if st.size > 0 else np.nan
        ...
        merge[~np.isnan(stim_times)] = stim_times[~np.isnan(stim_times)]
...
class ItiDuration(BaseBpodTrialsExtractor):
    ...
    **Optional:** saves _ibl_trials.iti_duration.npy
    ...
    def _extract(self):
        ...
        iti_dur = []
        for tr in self.bpod_trials:
            iti_times = tr['behavior_data']['States timestamps'].get('iti_period', None)
            if iti_times is None:
                iti_dur.append(np.nan)
            else:
                start, end = iti_times[0]
                iti_dur.append(end - start)

        return np.array(iti_dur)
...
class RuleCueTriggerTimes(BaseBpodTrialsExtractor):
    ...
    def _extract(self):
        trigger_times = np.zeros(len(self.bpod_trials))
        tone_states = ['standard', 'omission', 'random']

        for i, t in enumerate(self.bpod_trials):
            outcome = {sn: ~np.isnan(t['behavior_data']['States timestamps'].get(sn, [[np.nan]])[0][0]) for sn in tone_states}
            assert np.sum(list(outcome.values())) == 1
            outcome = next(k for k in outcome if outcome[k])
            trigger_times[i] = t['behavior_data']['States timestamps'][outcome][0][0]
        return trigger_times

class TrialType(BaseBpodTrialsExtractor):
    save_names = '_ibl_trials_feedback.type.npy'
    var_names = 'trial_feedback_type'

    def _extract(self):
        trial_type = []
        for tr in self.bpod_trials:
            #if ~np.isnan(tr["behavior_data"]["States timestamps"]["reward"][0][0]):
            if ~np.isnan(tr["behavior_data"]["States timestamps"].get("standard", [[np.nan]])[0][0]):
                trial_type.append(1)
            #elif ~np.isnan(tr["behavior_data"]["States timestamps"]["error"][0][0]):
            elif ~np.isnan(tr["behavior_data"]["States timestamps"].get("omission", [[np.nan]])[0][0]):
                trial_type.append(-1)
            #elif ~np.isnan(tr["behavior_data"]["States timestamps"]["no_go"][0][0]):
            elif ~np.isnan(tr["behavior_data"]["States timestamps"].get("random", [[np.nan]])[0][0]):
                trial_type.append(0)
            else:
...
class ItiInTimes(BaseBpodTrialsExtractor):
    ...
    def _extract(self):
        ...
        #     )
            ...
            iti_in = np.zeros(len(self.bpod_trials))
...
# class StimOffTimesDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_dummy.stimOff_times.npy',)
#     var_names = 'stimOff_times'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class StimFreezeTimesDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_dummy.stimFreeze_times.npy',)
#     var_names = 'stimFreeze_times'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class WheelTimestampsDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_wheel.timestamps.npy',)
#     var_names = 'wheel_timestamps'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class WheelPositionDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_wheel.position.npy',)
#     var_names = 'wheel_position'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class WheelMovesIntervalsDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_wheelMoves.intervals.npy',)
#     var_names = 'wheel_moves_intervals'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class WheelMovesPeakAmplitudeDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_wheelMoves.peakAmplitude.npy',)
#     var_names = 'wheel_moves_peak_amplitude'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class PeakVelocityDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_dummy.peakVelocity_times.npy',)
#     var_names = 'peakVelocity_times'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
    
# class IsFinalMovementDummy(BaseBpodTrialsExtractor):
#     save_names = ('_ibl_dummy.is_final_movement.npy',)
#     var_names = 'is_final_movement'
#     def _extract(self):
#         return np.zeros(len(self.bpod_trials))
...
class TrialsTable(BaseBpodTrialsExtractor):
    ...
    def _extract(self, extractor_classes=None, **kwargs):
        ...
        # base = [Intervals, RuleCueTimes, RuleCueTriggerTimes, FeedbackTimes, FeedbackType,
        #         RewardVolume, ItiDuration, ItiInTimes, TrialType,
        #         StimOffTimesDummy, StimFreezeTimesDummy, WheelTimestampsDummy, WheelPositionDummy,
        #         WheelMovesIntervalsDummy, WheelMovesPeakAmplitudeDummy, PeakVelocityDummy, IsFinalMovementDummy]
        base = [Intervals, RuleCueTimes, RuleCueTriggerTimes, FeedbackTimes, FeedbackType,
                RewardVolume, ItiDuration, ItiInTimes, TrialType]
        ...
        return table.to_df(), None
        # return {
        #     'table': AlfBunch({k: v for k, v in out.items() if k not in self.var_names}).to_df(),
        #     'stimOff_times': out['stimOff_times'],
        #     'stimFreeze_times': out['stimFreeze_times'],
        #     'wheel_timestamps': out['wheel_timestamps'],
        #     'wheel_position': out['wheel_position'],
        #     'wheel_moves_intervals': out['wheel_moves_intervals'],
        #     'wheel_moves_peak_amplitude': out['wheel_moves_peak_amplitude'],
        #     'peakVelocity_times': out['peakVelocity_times'],
        #     'is_final_movement': out['is_final_movement']
        # }
...
def extract_all(session_path, save=False, bpod_trials=None, settings=None, task_collection='raw_behavior_data', save_path=None):
    ...
    # base = [RepNum, RuleCueTriggerTimes]
    base = []
    # Version check
    if parse_version(settings['IBLRIG_VERSION_TAG']) >= parse_version('5.0.0'):
        ...
        #     ErrorCueTriggerTimes, TrialsTable, PhasePosQuiescence, TrialMod
        # ])
        base.extend([FeedbackType, RewardVolume, FeedbackTimes, Intervals, ItiDuration,
                    RuleCueTriggerTimes, TrialType, RuleCueTimes, ItiInTimes, TrialsTable
        ])
    else:
        base.extend([FeedbackType, RewardVolume, FeedbackTimes, Intervals, ItiDuration,
                    RuleCueTriggerTimes, TrialType, RuleCueTimes, ItiInTimes, TrialsTable
