...
class TrialCon(BaseBpodTrialsExtractor):
    """
    1 is conflict, 0 is uni modal
    
    """
    save_names = '_ibl_trials_condition.type.npy'
    var_names = 'condition'

    def _extract(self):
        trialcon = np.zeros(len(self.bpod_trials), np.int64)
        for i, t in enumerate(self.bpod_trials):
            trialcon[i] = int(t['trial_type'][0])
        return trialcon
...
class TrialsTable(BaseBpodTrialsExtractor):
    ...
    save_names = ('_ibl_trials.table.pqt',None, None)
    var_names = ('table','stimOff_times', 'stimFreeze_times')
    ...
    def _extract(self, extractor_classes=None, **kwargs):
        base = [Intervals, RuleCueTimes, RuleCueTriggerTimes, ResponseTimes, Choice, StimOnTriggerTimes, StimOnOffFreezeTimes, ContrastLR, FeedbackTimes, FeedbackType,
                RewardVolume, ProbabilityLeft,  TrialMod, TrialCon, Omission]
...
def extract_all(session_path, save=False, bpod_trials=None, settings=None, task_collection='raw_behavior_data', save_path=None):
    ...
    if parse_version(settings['IBLRIG_VERSION_TAG']) >= parse_version('5.0.0'):
        ...
        base.extend([
            ...
            ErrorCueTriggerTimes, TrialsTable, PhasePosQuiescence, TrialMod, TrialCon
        ])
    else:
        base.extend([
            Intervals, Wheel, FeedbackType, ContrastLR, ProbabilityLeft, Choice, IncludedTrials,
            StimOnTimes_deprecated, RewardVolume, FeedbackTimes, ResponseTimes, RuleCueTimes, PhasePosQuiescence, TrialMod, TrialCon
        ])
